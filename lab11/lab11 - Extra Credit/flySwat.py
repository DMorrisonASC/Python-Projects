...
class Fly(pygame.sprite.Sprite):
    ...
    def loadImage(self):
        self.flySheet = pygame.image.load("flySheet.png")
        self.flySheet = self.flySheet.convert()

        self.imgList = []
        
        imgSize = (49, 58)
        offset = ((1, 1), (50, 1), (99, 1), (157, 1))

        for i in range(4) :
            self.changeImg = pygame.Surface(imgSize)
            self.changeImg.blit(self.flySheet, (0, 0), (offset[i], imgSize))
            self.transColor = self.changeImg.get_at((1,1))
            self.changeImg.set_colorkey(self.transColor)
            self.imgList.append(self.changeImg)   
